...
export class M1764161281643AddCategoryDescription1764161283727
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`category\` ADD \`description\` text NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`category_info\` ADD \`category_id\` varchar(36) NOT NULL`,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX \`REL_9228deaf3cc55aa0d37afd344b\` ON \`category_info\` (\`category_id\`)`,
    );
